    ...
    public function runSabcTest(Request $request, $test)
    {
        ...
        switch (strtolower($environment)) {
            case 'production':
                ...
                $env_url = '';
                $db_connection = "host=" . env("DB_SABC_HOST_PROD") . " port=" . env("DB_SABC_PORT_PROD") . " dbname=" . env("DB_SABC_DATABASE_PROD") . " user=" . env("DB_SABC_USERNAME_PROD") . " password=" . env("DB_SABC_PASSWORD_PROD");
                break;
            case 'uat':
                $params = ['applicationNumber' => "".env('UAT_SABC_WSDL_APP')."", 'userGUID' => "".env('UAT_SABC_WSDL_GUID').""];
                $env_url = 'uat.';
                $db_connection = "host=" . env("DB_SABC_HOST_UAT") . " port=" . env("DB_SABC_PORT_UAT") . " dbname=" . env("DB_SABC_DATABASE_UAT") . " user=" . env("DB_SABC_USERNAME_UAT") . " password=" . env("DB_SABC_PASSWORD_UAT");
                break;
            case 'dev':
                $params = ['applicationNumber' => "".env('DEV_SABC_WSDL_APP')."", 'userGUID' => "".env('DEV_SABC_WSDL_GUID').""];
                $env_url = 'dev.';
                $db_connection = "host=" . env("DB_SABC_HOST_DEV") . " port=" . env("DB_SABC_PORT_DEV") . " dbname=" . env("DB_SABC_DATABASE_DEV") . " user=" . env("DB_SABC_USERNAME_DEV") . " password=" . env("DB_SABC_PASSWORD_DEV");
                break;
        }
        ...
        if($test->dusk_test === false) {
            ...
            if($result['status'] != 200){
                ...
                Log::channel('monitor')->info("SABC Test: " . $service . " on the env (" . $test->env . ") failed. Number of attempts: " . $attempt);
